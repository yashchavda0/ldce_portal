...
const ChangeDepartmentPassword = [
  body("password")
    .notEmpty()
    .withMessage("password is required")
    .isLength({ min: 8 })
    .withMessage("password must be atleast 8 characters"),
  body("username").notEmpty().withMessage("You Must be Logged In "),
  ...
  async (req, res) => {
    ...
    if (errors.isEmpty()) {
      try {
        ...
        const user = await DepartmentQueries.checkUsername(
...
module.exports = ChangeDepartmentPassword;
